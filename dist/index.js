...
/* harmony import */ var child_process__WEBPACK_IMPORTED_MODULE_4__ = __nccwpck_require__(317);
...
function runCmd(cmd, options = {}) {
  core.info(`$ ${cmd}`);
  try {
    (0,child_process__WEBPACK_IMPORTED_MODULE_4__.execSync)(cmd, { stdio: 'inherit', ...options });
  } catch (err) {
...
function readJson(filePath) {
  const raw = fs__WEBPACK_IMPORTED_MODULE_1__.readFileSync(filePath, 'utf8');
...
async function main() {
  try {
    ...
    const tmpDir = process.env.RUNNER_TEMP || os__WEBPACK_IMPORTED_MODULE_3__.tmpdir();
    const workDir = process.cwd();
    const analyzerDir = path__WEBPACK_IMPORTED_MODULE_2__.join(tmpDir, 'cloudcost-analyzer');
    const analyzerPath = path__WEBPACK_IMPORTED_MODULE_2__.join(analyzerDir, 'analyzer');
    const headJson = path__WEBPACK_IMPORTED_MODULE_2__.join(tmpDir, 'cloudcost-head-report.json');
    const baseJson = path__WEBPACK_IMPORTED_MODULE_2__.join(tmpDir, 'cloudcost-base-report.json');

    fs__WEBPACK_IMPORTED_MODULE_1__.mkdirSync(analyzerDir, { recursive: true });

    const analyzerUrl = `https://github.com/odrori1997/cloudcost-analyzer/archive/refs/tags/${analyzerVersion}.tar.gz`;
    const archivePath = path__WEBPACK_IMPORTED_MODULE_2__.join(analyzerDir, 'analyzer.tar.gz');
    ...
    runCmd(
      `"${analyzerPath}" ` +
        ...
        `--out-json "${headJson}" ` +
        `--out-md "${path__WEBPACK_IMPORTED_MODULE_2__.join(tmpDir, 'cloudcost-head-report.md')}" ` +
        `--api-key "${apiKey}" ` +
    ...
    runCmd(
      `"${analyzerPath}" ` +
        ...
        `--usage-profile ${usageProfile} ` +
        `--out-json "${baseJson}" ` +
        `--out-md "${path__WEBPACK_IMPORTED_MODULE_2__.join(tmpDir, 'cloudcost-base-report.md')}" ` +
        `--api-key "${apiKey}" ` +
        `--backend-url "${backendUrl}"`,
      { cwd: workDir },
    );
